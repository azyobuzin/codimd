...
// Verify that sourceURL is correct
if (env === Environment.production) {
  request.get(config.sourceURL)
    .on('error', function (err) {
      logger.error('Could not verify URL to your instance\'s source code. Be aware that you may violate AGPL.\nGet some more information about AGPL here: https://choosealicense.com/licenses/agpl-3.0/ \n Error: ' + err)
    })
    .on('response', function (response) {
      if (response.statusCode !== 200) {
        logger.error('Your settings for the `sourceURL` or `CMD_SOURCE_URL` are wrong. This violates AGPL! Please fix your repository settings!\nGet some more information about AGPL here: https://choosealicense.com/licenses/agpl-3.0/')
        process.exit(1)
      }
    })
}
